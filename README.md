...
* 之前使用的是拦截器， 然后检查是否使用@LoginRequired注解

#### 授权配置

* 对当前系统内所包含的所有请求，分配权限访问权限（普通用户、版主、管理员）

#### 认证方案

* 绕过security认证流程，采用系统原来认证方案
...
* 避免缓存雪崩（缓存失效、大量缓存直达DB），提高系统的可用性

![缓存](https://i.loli.net/2020/05/08/l43bBjUJVnYPSIz.png)

#### 添加一级的缓存前后对别（使用Jmeter）

![性能测试-未做一级缓存](https://i.loli.net/2020/05/08/l3E8hLRGWtDji2a.png)
